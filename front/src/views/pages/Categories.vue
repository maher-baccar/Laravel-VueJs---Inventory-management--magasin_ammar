      ...
      <v-toolbar flat>
        ...
        <v-dialog v-model="dialog" max-width="500px">
          <template v-slot:activator="{ on, attrs }">
            <v-btn color="primary" dark class="mb-2" v-bind="attrs" v-on="on">Ajouter Une Catégorie</v-btn>
          </template>
          <v-card>
            <v-card-title>
...
export default {
  ...
  computed: {
    formTitle() {
      return this.editedIndex === -1 ? 'Nouveau Catégorie' : 'Modifier Catégorie'
    },
  },
  ...
  methods: {
    ...mapActions('admin', ['All_CATEGORIES', 'ADD_CATEGORY', 'DELETE_CATEGORY','UPDATE_CATEGORY']),
    initialize() {
      this.loader = true
      this.All_CATEGORIES()
        .then(res => {
    ...
    save() {
      if (this.editedIndex > -1) {
        this.UPDATE_CATEGORY({ id: this.editedItem.id, payload: this.editedItem })
          .then(res => {
            this.initialize()
          })
          .catch(e => {
