<template>
  ...
  >
    <template v-slot:top>
      <v-toolbar flat>
        <v-toolbar-title>Mes Magasiniers</v-toolbar-title>
        ...
        <v-dialog v-model="dialog" max-width="500px">
          <template v-slot:activator="{ on, attrs }">
            <v-btn color="primary" dark class="mb-2" v-bind="attrs" v-on="on">Ajouter Un Magasinier</v-btn>
          </template>
          <v-card>
            <v-card-title>
...
export default {
  ...
  methods: {
    ...mapActions('admin', ['All_STOREKEEPERS', 'ADD_STOREKEEPER', 'UPDATE_STOREKEEPER','DELETE_STOREKEEPER']),
    initialize() {
      this.loader = true
      this.All_STOREKEEPERS()
      .then(res => {
          this.items = [...res]
          this.items.map(el => {
    ...
    deleteItemConfirm() {
      this.DELETE_STOREKEEPER(this.editedItem.id)
        .then(res => {
            console.log(res);
          this.initialize()
          this.$swal({
    ...
    save() {
      if (this.editedIndex > -1) {
        this.UPDATE_STOREKEEPER({ id: this.editedItem.id, payload: this.editedItem })
          .then(res => {
            this.initialize()
          })
          .catch(e => {
        ...
        this.ADD_STOREKEEPER(this.editedItem)
          .then(res => {
            this.initialize()
          })
          .catch(e => {
